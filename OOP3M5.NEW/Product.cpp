...
namespace AMA{
  ...
  std::fstream& Product::store(std::fstream& file, bool newLine)const{
    file << type_ << "," << sku() << "," << name() << "," <<  unit()  << "," <<
          taxed() << "," << price() << "," << quantity() << "," << qtyNeeded();
    if (newLine) 
      file << endl;
    return file;
  }
  ...
  std::fstream& Product::load(std::fstream& file){
    ...
    file.getline(sku_1, max_sku_length+1,',');                       
    file.getline(nameProd_1, max_name_length+1,',');                  
    file.getline(unitDesc_1, max_unit_length+1,',');                 

    file >> taxable_1 >> comma >> priceSingleUnit_1 >> comma >>
            numOnHand_1 >> comma >> numNeeded_1;
